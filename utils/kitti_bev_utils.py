...
def inverse_yolo_target_v2(targets, bc):
    ...
    for t in targets:
        ...
        labels[n, :] = c, x, y, z, h, w, l, - np.arctan2(im, re) - 2*np.pi, conf
        n += 1
...
def drawRotatedBox(img,x,y,w,l,yaw,color):
    ...
    cv2.line(img, (corners_int[0, 0].round().astype(int), corners_int[0, 1].round().astype(int)), (corners_int[3, 0].round().astype(int), corners_int[3, 1].round().astype(int)), (255, 255, 0), 2)
