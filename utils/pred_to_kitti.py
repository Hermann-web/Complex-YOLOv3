...
def predictions_to_kitti_format(img_detections:List[torch.Tensor], calib:kitti_utils.Calibration, img_shape_2d, img_size:int, RGB_Map=None):
...
def predictions_to_kitti_format_v2(img_detections:List[torch.Tensor], calib:kitti_utils.Calibration, img_shape_2d, img_size:int, RGB_Map=None):
    ...
    predictions = bev_utils.inverse_yolo_target_v2(predictions, cnf.boundary)
    if predictions.shape[0]:
